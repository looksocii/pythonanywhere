...
class MerkleTools(object):
    ...
    def _calculate_next_level(self):
        ...
        for l, r in zip(self.levels[0][0:N:2], self.levels[0][1:N:2]):
            value = '0x01' + self.hash_function(l.encode('utf-8') + r.encode('utf-8')).hexdigest()
            new_level.append(value)
            # add the data (l, r) to the list (nodes)
            if length == 1:
                leftNode = Node(None, None, l, None, None)
                rightNode = Node(None, None, r, None, None)
                self.nodes.append(leftNode)
                self.nodes.append(rightNode)
            node = Node(None, None, value, l, r)
            self.nodes.append(node)
        if solo_leave is not None:
            value = '0x01' + self.hash_function(solo_leave.encode('utf-8') + solo_leave.encode('utf-8')).hexdigest()
            new_level.append(value)
            # add the data (solo_leave) to the list (nodes)
            if length == 1:
                leftNode = Node(None, None, solo_leave, None, None)
                rightNode = Node(None, None, solo_leave, None, None)
                self.nodes.append(leftNode)
                self.nodes.append(rightNode)
            node = Node(None, None, value, solo_leave, solo_leave)
            self.nodes.append(node)
